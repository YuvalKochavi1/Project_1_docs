...
%-----------------------------------------------------------------------------
\subsection{Slide: Radiative Heat Waves in HEDP}
%-----------------------------------------------------------------------------

\begin{slidebox}
\textbf{Slide title:} Radiative Heat Waves in HEDP
\end{slidebox}

To begin, radiative heat waves are a highly common and important phenomenon in High Energy Density Physics, or HEDP. We see them primarily in two main areas: inertial confinement fusion and astrophysics.

On the left side of the slide, we have Inertial Confinement Fusion, or ICF, such as the experiments conducted at the National Ignition Facility, or NIF. In these setups, radiation driven in a Hohlraum heats a low-density foam, causing supersonic heat waves to propagate through the material before any significant hydrodynamic motion can take place.

On the right side, we see astrophysics. Radiative heat waves and transport are crucial for understanding energy transport in stars, and they govern the dynamics of radiation-dominated stellar structures and supersonic shocks in space.

This wide range of applications highlights why it is so important to have a solid physical understanding of these waves, which leads us to our specific research motivation.
...
From the RTE, after taking angular moments and applying the diffusion approximation, we arrive at the so-called ``gray'' coupled equations. The first relation governs the radiation energy density $E$, which evolves through diffusion and through coupling to the material via the opacity $\sigma$. The second relation governs the material internal energy $e$, which exchanges energy with the radiation field.

Now, a key simplification: if we assume complete thermodynamic equilibrium, or CTE, between the radiation field and the material --- meaning $T_R = T_m$ --- we can combine these two relations into a single nonlinear equation. This is the final equation shown at the bottom of the slide, which describes the combined transport and heating.
...
%-----------------------------------------------------------------------------
\subsection{Slide: Marshak Waves Experiments}
%-----------------------------------------------------------------------------
...
On the left you can see the drive temperature profile as a function of time, based on Back et al. POP 2000. On the right you see a schematic of the experimental geometry from Cohen et al. PRR 2018: the Hohlraum generates a radiation field that enters the foam tube, and the heat wave propagates along the tube axis.

%-----------------------------------------------------------------------------
\subsection{Slide: Marshak Waves Experimental Setup}
%-----------------------------------------------------------------------------

\begin{slidebox}
\textbf{Slide title:} Marshak Waves Experimental Setup
\end{slidebox}

There are several ways to track the heat wave propagation in experiments, which we summarize on this slide.

On the left side, we show three different diagnostic methods used to track the heat front position and the emerging radiation, with data from Back et al. POP 2000, Rosen et al. ASS 2006, and Cohen et al. PRR 2018.

On the right side, we show the resulting measurements: the radiation flux emerging from the rear of the foam, which gives us the breakout time, and the spatial curvature of the emerging flux, which reveals the two-dimensional structure of the heat wave. Both of these plots are also from the work of Back et al. POP 2000.
...
Let me now move on to what we call our 1.5D model. The idea is to start from the one-dimensional Hammer--Rosen analytic solution and systematically incorporate two-dimensional effects as corrections, hence the name ``1.5D.''
...
Here we discuss the governing equations of the Hammer--Rosen model. Hammer and Rosen solved the diffusion equation by using power-law descriptions for the material properties: specifically, specific energy $e$ and opacity $\kappa$ as power laws of temperature and density, governed by parameters $f$, $\mu$, $g$, $\lambda$, $\alpha$, and $\beta$.

By assuming that the material energy density dominates over the radiation energy density --- which is a valid assumption up to temperatures of around 3~keV --- the radiation energy storage term can be neglected. This simplifies the governing relation to the nonlinear diffusion equation shown in the middle of the slide.

Using perturbation theory, Hammer and Rosen solved this simplified equation to derive key physical quantities, as detailed in Rosen et al., Physics of Plasmas, 2003.
...
The key result from the Hammer--Rosen model is this formula for the heat front position $x_F$ as a function of time. The front position squared is given by an integral expression involving the surface temperature history through the function $H(t) = T_s^{4+\alpha}$.
...
This is a very significant discrepancy, and it tells us that the naive 1D model is missing important physics. So the question is: what effects are we neglecting?
...
The first and most important 1D correction is the boundary condition. In the original HR model, the surface temperature $T_s$ was assumed to equal the drive temperature $T_D$. But this is not physical.

In reality, the Marshak boundary condition states that the drive temperature is related to the angular integral of the specific intensity at the boundary. This is expressed in terms of the radiation energy density $E(0,t)$ and the net flux $F(0,t)$ entering the foam, as shown in the first equation.

This leads to the relation where the drive temperature flux equals the surface emission flux plus half the entering flux. Therefore, because energy is being carried away into the foam, the surface temperature $T_s$ is always \textbf{lower} than the drive temperature $T_D$. Using $T_D$ directly in the HR model overestimates the drive, leading to an artificially fast front.
...
As shown in the figures at the bottom of the slide, we see the transition of the effective mean free path. On the left, we compare the effective mean free path $\lambda_{\mathrm{eff}}$ profiles over time for different power values ($n=1$, $n=2$, and $n=5$), showing how they approach the Rosseland limit at early times and are bounded by the geometric limit $2R$ at later times. On the right, we show the transition curve from the model.
...
%-----------------------------------------------------------------------------
\subsection{Slide: Section Title --- Comparison with Experimental Data}
%-----------------------------------------------------------------------------

\begin{slidebox}
\textbf{Slide title:} Full 2D model --- Comparison with Experimental Data (section divider)
\end{slidebox}

Now we will look at the comparison of our model directly with experimental data for both radiation breakout flux shapes and arrival time distributions.
