...
Radiative heat waves are central to inertial-confinement fusion (ICF) and high-energy-density physics~\cite{Rosen1996}. In the indirect-drive approach, laser energy is converted into x~rays inside a high-$Z$ hohlraum that heat and implode the fusion capsule~\cite{Lindl2004}. Because radiation modifies the temperature, ionization state, and opacity of the medium, energy storage and transport are strongly coupled and nonlinear. Additional complexity arises from plasma filling and laser--plasma interactions, which can reduce the energy delivered to the hohlraum wall so that the applied laser pulse does not translate directly into the radiation drive seen by the target~\cite{Hinkel2006PRL}.

Motivated by this broader hohlraum-radiation context, a major experimental step was made by Afshar-rad \textit{et al.}, who reported direct measurements of the sustained propagation of a supersonic, radiation-driven heat wave in low-density foam~\cite{AfsharRad1994}. Lowering the density allowed the radiative front to remain ahead of the hydrodynamic response, while the measurements showed that re-emission from the heated foam plays an essential role in the propagation. However, these early experiments generally probed distances of order one photon mean free path and were only weakly supersonic, placing them outside the regime most naturally described by diffusion theory.

Back \textit{et al.} extended these studies to foams several optical depths thick and reported the first detailed measurements of a diffusive, supersonic radiation wave in a finite cylindrical target~\cite{Back2000PRL,Back2000POP}. The wave broke out first on axis and later near the boundary, revealing that energy loss to the surrounding walls bends and retards the front. These experiments demonstrated that opacity, wall material, and transverse geometry jointly govern heat-wave propagation. Subsequent campaigns extended these results to a wider range of materials, energies, sleeves, and geometries~\cite{Moore2015,Courtois2024,Keiter2008,Jiang2005,Zhang2010}. 

In parallel, Hammer and Rosen developed a self-consistent one-dimensional theory for nonlinear Marshak waves under arbitrary drive histories~\cite{Hammer2003}. Subsequent studies showed that quantitative agreement with experiment requires self-consistent boundary conditions together with two-dimensional energy-balance and hydrodynamic corrections~\cite{Cohen2018,Cohen2020Key,Courtois2022,Courtois2024}.

In this Letter, we synthesize these developments into a semi-analytic two-dimensional model for supersonic Marshak-wave propagation in finite cylindrical foam targets. Building on the Hammer--Rosen framework, we include a self-consistent radiative boundary condition, material-dependent thermodynamic and opacity closures, wall-coupled energy loss and hydrodynamic effects, and a finite-size transport correction that connects the diffusion and wall-collision limits. We then couple the corrected longitudinal evolution to a cylindrical eigenmode description of the transverse front~\cite{Hurricane2006,Courtois2022}, with curvature set by a time-dependent wall albedo. The resulting front geometry and re-emission profile are benchmarked against representative experiments and two-dimensional diffusion simulations.
...
\begin{figure}[ht]
    ...
    \begin{minipage}{0.5\columnwidth}
        ...
        \centerline{(b)}
    \end{minipage}
    \caption{(a) Experimental platform: laser beams enter the hohlraum through the LEH, generate x rays, and drive a Marshak wave in low-density foam targets of different lengths~\cite{Moore2015}. (b) Measured hohlraum drive temperature $T_D(t)$ for the high-energy Back experiment; the dashed curve is the normalized laser pulse shape~\cite{Back2000PRL}.}
...
The longitudinal front position captures only axial propagation, whereas the physical front is curved, primarily by radiative losses to the sleeve. This curvature is experimentally important because the measured signal is the re-emission intensity, $\Phi\propto T^4$, which strongly amplifies transverse temperature variations~\cite{Back2000POP,Back2000PRL}. As a result, the intrinsically 2D front shape both delays breakout at the diagnostic plane and feeds back on the energy balance, since wall losses are controlled by the boundary-front evolution at $r=R$.
...
\begin{figure}[t]
    ...
    \begin{minipage}{0.4\columnwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/diff_albedo.png}
        \vspace{-0.8em}
        \centerline{(a)}
    \end{minipage}\hfill
    \begin{minipage}{0.4\columnwidth}
