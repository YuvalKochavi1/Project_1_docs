...
\subsection{Key step: ``fit Eq.(23) into Eq.(21)'' via harmonic face diffusion}
...
At the outer boundary ($r=R$), either a Neumann condition ($\partial E/\partial r=0$)
or a Dirichlet condition ($E=E_{\mathrm{bath}}$) is imposed by appropriately modifying
the outward flux term. but it shouldn't matter a lot here where the gold region is applied.
